...
export async function GET(_req: NextRequest, { params }: RouteContext): Promise<NextResponse> {
  try {
    ...
    const [itemsResult, havenResult] = await Promise.all([
      pool.query(
        `SELECT id, haven_id, name, icon, icon_url, price_per_night, is_active, created_at
         FROM haven_rentable_items
         WHERE haven_id = $1 AND is_active = true
         ORDER BY id ASC`,
        [id],
      ),
      pool.query<{ partner_id: string | null }>(
...
export async function POST(req: NextRequest, { params }: RouteContext): Promise<NextResponse> {
  try {
    const { id } = await params;
    const { name, icon, icon_url, price_per_night } = await req.json();
    ...
    const result = await pool.query(
      `INSERT INTO haven_rentable_items (haven_id, name, icon, icon_url, price_per_night)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, haven_id, name, icon, icon_url, price_per_night, is_active`,
      [id, name.trim(), (icon || "default").trim(), icon_url || null, parseFloat(price_per_night)],
    );
