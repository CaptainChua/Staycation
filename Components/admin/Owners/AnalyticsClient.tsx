...
export default function AnalyticsClient({ summary, revenueByHaven, monthlyRevenue }: AnalyticsClientProps) {
  ...
  // Create a map of months from backend data
  const revenueMap = new Map<string, number>();
  monthlyRevenue.forEach(m => {
    const monthStr = m.month.toLowerCase();
    const monthIndex = monthNames.findIndex(name => name.toLowerCase().startsWith(monthStr.substring(0, 3)));
    if (monthIndex >= 0) {
      revenueMap.set(monthNames[monthIndex], m.revenue);
    }
  });

  // Create complete 12-month array with all months
  const xLabels = monthNames;
  const revenueData = monthNames.map(month => revenueMap.get(month) ?? null);
  ...
  const lineOptions = {
    ...
    scales: {
      x: {
        grid: { color: '#e5e5e5', drawBorder: true },
        ticks: { 
          font: { size: 12 },
          maxRotation: 45,
          minRotation: 0,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      {/* Advanced Analytics Line Graph - Full Width Chart */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-900 overflow-hidden w-full">
        <div className="p-6 border-b-2 border-gray-200 dark:border-gray-600">
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Revenue</h2>
        </div>
        <div className="p-4 flex items-center justify-center" style={{ minHeight: '280px' }}>
          {monthlyRevenue.length === 0 ? (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              No analytics data available.
            </div>
          ) : (
            <Line data={lineData} options={{ ...lineOptions, maintainAspectRatio: true }} />
          )}
        </div>
      </div>
    </div>
