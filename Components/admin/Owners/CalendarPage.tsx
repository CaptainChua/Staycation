...
import { Calendar, ChevronLeft, ChevronRight, Filter, X, User, MapPin, CalendarDays, DollarSign, Lock } from "lucide-react";
...
const CalendarPage = ({ havens }: CalendarPageProps) => {
  ...
  // Enable polling to auto-refresh when new bookings are created
  const { data: bookings = [], isLoading, error } = useGetBookingsQuery(
    {},
    {
      pollingInterval: 5000, // Refresh every 5 seconds to show new bookings immediately
  ...
  // Fetch blocked dates for all havens
  const { data: blockedDatesData = { data: [] } } = useGetBlockedDatesQuery(
    {},
    {
  ...
  // Filter blocked dates to show only those for selected haven
  const filteredBlockedDates = useMemo(() => {
    if (!selectedHaven) return [];
    
    const allBlockedDates = blockedDatesData?.data || [];
    return allBlockedDates.filter(blockedDate => {
      return blockedDate.haven_id === selectedHaven.uuid_id;
    });
  }, [blockedDatesData, selectedHaven]);
  ...
  const calendarEvents: CalendarEvent[] = useMemo(() => {
    ...
    const bookingEvents = bookingsToUse.map((booking: Booking) => {
    ...
    // Convert blocked dates to events
    const blockedDateEvents = filteredBlockedDates.map((blockedDate, index) => {
      // Parse dates more accurately
      const [year, month, day] = blockedDate.from_date.split('T')[0].split('-').map(Number);
      const [endYear, endMonth, endDay] = blockedDate.to_date.split('T')[0].split('-').map(Number);
      
      const fromDateStr = `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
      const endDateObj = new Date(endYear, endMonth - 1, endDay);
      endDateObj.setDate(endDateObj.getDate() + 1); // Add 1 day to make it inclusive
      const toDateStr = `${endDateObj.getFullYear()}-${String(endDateObj.getMonth() + 1).padStart(2, '0')}-${String(endDateObj.getDate()).padStart(2, '0')}`;

      return {
        id: `blocked-${blockedDate.id}-${index}`,
        title: blockedDate.reason ? `BLOCKED - ${blockedDate.reason}` : 'BLOCKED',
        start: fromDateStr,
        end: toDateStr,
    ...
    return [...bookingEvents, ...blockedDateEvents];
  }, [filteredBookings, filteredBlockedDates, filterStatus]);

  // Handle event click
  const handleEventClick = (clickInfo: EventClickArg) => {
    const { isBlockedDate } = clickInfo.event.extendedProps;
    
    // Don't open modal for blocked dates
    if (isBlockedDate) {
      return;
    }
  ...
  const renderEventContent = (eventInfo: EventContentArg) => {
    const { duration, isBlockedDate, reason } = eventInfo.event.extendedProps;
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900 p-4 flex-shrink-0 border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600 dark:text-gray-400">Blocked/Unavailable</span>
          </div>
        </div>
      </div>
