...
import { parseDate, getLocalTimeZone, today, DateValue, Time } from "@internationalized/date";
...
interface NewReservationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (bookingData: Record<string, unknown>) => Promise<void>;
  blockedDates?: BlockedDate[];
}

const NewReservationModal = ({ isOpen, onClose, onSubmit, blockedDates = [] }: NewReservationModalProps) => {
  const [currentStep, setCurrentStep] = useState(1);
  const [completedSteps, setCompletedSteps] = useState<number[]>([]);
  const [errors, setErrors] = useState<Record<string, string>>({});
  // Removed unused errorRefs
  ...
  // numberOfDays is not used directly
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'payment' | 'id', guestIndex?: number) => {
    ...
    if (file) {
      ...
      // Convert file to base64 (including data URI prefix for Cloudinary)
      const reader = new FileReader();
      reader.onload = (event) => {
        const base64DataUri = event.target?.result as string;
        ...
        if (type === 'payment') {
          setFormData((prev) => ({
            ...
            paymentProofBase64: base64DataUri,
          }));
        } else if (guestIndex === undefined) {
          setFormData((prev) => ({
            ...prev,
            validId: file,
            validIdPreview: previewUrl,
            validIdBase64: base64DataUri,
          }));
        } else {
          const updatedGuests = [...additionalGuests];
          updatedGuests[guestIndex].validId = file;
          updatedGuests[guestIndex].validIdPreview = previewUrl;
          updatedGuests[guestIndex].validIdBase64 = base64DataUri;
  ...
  // Helper function to convert file to base64 promise (with data URI prefix for Cloudinary)
  ...
  const checkBlockedDateConflict = (checkInStr: string, checkOutStr: string): string | null => {
    if (!checkInStr || !checkOutStr || blockedDates.length === 0) return null;
    
    const checkInDate = new Date(checkInStr);
    const checkOutDate = new Date(checkOutStr);
    
    for (const blockedDate of blockedDates) {
      const blockedFrom = new Date(blockedDate.from_date.split('T')[0]);
      const blockedTo = new Date(blockedDate.to_date.split('T')[0]);
      blockedTo.setDate(blockedTo.getDate() + 1); // Include the to_date
      
      // Check for overlap: checkIn < blockedTo AND checkOut > blockedFrom
      if (checkInDate < blockedTo && checkOutDate > blockedFrom) {
        return `Cannot book: Dates overlap with blocked period (${blockedDate.reason || 'Blocked'})`;
      }
    }
    return null;
  };
  ...
  const validateStep2 = (): boolean => {
    ...
    // Check for blocked date conflicts
    const blockedConflict = checkBlockedDateConflict(formData.checkInDate, formData.checkOutDate);
    if (blockedConflict) {
      newErrors.checkInDate = blockedConflict;
    }
  ...
  const validateStep4 = (): boolean => {
    ...
    if (!formData.paymentProofBase64) newErrors.paymentProof = "Please wait for payment proof to upload";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Generate booking ID
    // Generate booking ID (safe in event handler)
    const bookingId = `BK-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;

    // Format additional guests for API
    const formattedAdditionalGuests = additionalGuests.map(guest => ({
      firstName: guest.firstName,
      lastName: guest.lastName,
      age: guest.age,
      gender: guest.gender,
      validId: guest.validIdBase64 || null,
      email: undefined, // Optional - will fallback to main guest email
      phone: undefined, // Optional - will fallback to main guest phone
    }));

    const bookingData = {
      booking_id: bookingId,
      user_id: null, // Will be set by the API or admin context
      ...
      valid_id: formData.validIdBase64 || null, // base64 data
      // Additional guests
      additional_guests: formattedAdditionalGuests,
      // Payment info
      payment_method: formData.paymentMethod,
      payment_proof: formData.paymentProofBase64 || null, // base64 data
      room_rate: roomRate,
  ...
  // Helper: block unavailable dates for DatePicker
  function isDateBlocked(dateObj: DateValue) {
    let jsDate: Date | null = null;
    if (typeof dateObj === 'object' && dateObj !== null && 'toDate' in dateObj && typeof dateObj.toDate === 'function') {
      jsDate = dateObj.toDate(getLocalTimeZone());
    } else if (
      typeof dateObj === 'object' &&
      dateObj !== null &&
      typeof (dateObj as { year?: unknown }).year === 'number' &&
      typeof (dateObj as { month?: unknown }).month === 'number' &&
      typeof (dateObj as { day?: unknown }).day === 'number'
    ) {
      const d = dateObj as { year: number; month: number; day: number };
      jsDate = new Date(d.year, d.month - 1, d.day);
    }
    if (!jsDate) return false;
    const dateStr = jsDate.toISOString().split('T')[0];
    // Block dates that fall within blocked date ranges
    for (const blocked of blockedDates) {
      const from = blocked.from_date.split('T')[0];
      const to = blocked.to_date.split('T')[0];
      if (dateStr >= from && dateStr <= to) return true;
    }
    return false;
  }

  // Helper: block unavailable times for TimeInput (half-hour granularity)
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <DatePicker
                          ...
                          value={formData.checkInDate ? parseDate(formData.checkInDate) : null}
                          onChange={date => {
                              let dateStr = "";
                              if (date) {
                                // Extract year, month, day directly to avoid timezone issues
                                const year = (date as { year?: number }).year;
                                const month = (date as { month?: number }).month;
                                const day = (date as { day?: number }).day;
                                if (year && month && day) {
                                  dateStr = `${year.toString().padStart(4, '0')}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
                                }
                              }
                              setFormData(prev => ({ ...prev, checkInDate: dateStr }));
                              // Optionally reset check-out date if before check-in
                              if (formData.checkOutDate && formData.checkOutDate <= dateStr) {
                          ...
                          className={inputClass}
                          isRequired
                          isDisabled={!formData.stayType || !formData.roomName}
                        />
                      </div>
                      <div>
                        ...
                        <DatePicker
                          ...
                          value={formData.checkOutDate ? parseDate(formData.checkOutDate) : null}
                          onChange={date => {
                              let dateStr = "";
                              if (date) {
                                // Extract year, month, day directly to avoid timezone issues
                          ...
                          isDateUnavailable={isDateBlocked}
                          minValue={formData.checkInDate ? parseDate(formData.checkInDate).add({ days: 1 }) : today(getLocalTimeZone()).add({ days: 1 })}
                          maxValue={today(getLocalTimeZone()).add({ days: 91 })}
                          className={inputClass}
                          isRequired
                          isDisabled={!formData.stayType || !formData.roomName}
                        />
                      ...
                      <div>
                        ...
                        <TimeInput
                          ...
                          value={formData.checkInTime ? new Time(...formData.checkInTime.split(":").map(Number)) : null}
                          onChange={time => setFormData(prev => ({ ...prev, checkInTime: time ? `${time.hour.toString().padStart(2, "0")}:${time.minute.toString().padStart(2, "0")}` : "" }))}
                          granularity="minute"
                          className={inputClass}
                          isRequired
                          isDisabled={!formData.stayType || !formData.roomName}
                      ...
                      <div>
                        ...
                        <TimeInput
                          ...
                          value={formData.checkOutTime ? new Time(...formData.checkOutTime.split(":").map(Number)) : null}
                          onChange={time => setFormData(prev => ({ ...prev, checkOutTime: time ? `${time.hour.toString().padStart(2, "0")}:${time.minute.toString().padStart(2, "0")}` : "" }))}
                          granularity="minute"
                          className={inputClass}
                          isRequired
                          isDisabled={!formData.stayType || !formData.roomName}
