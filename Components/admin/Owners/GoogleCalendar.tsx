...
export default function GoogleCalendarPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      type SyncResultItem = { status: string; html_link?: string; calendar_id?: string };
      type ExtendedResult = typeof result & { results?: SyncResultItem[]; errors?: string[] };
      const extended = result as ExtendedResult;
      const firstSynced = extended.results?.find((r) => r.status === "synced");
      setSyncResult({
      ...
      const detail = apiErr?.data?.error || apiErr?.data?.message || "Failed to sync bookings to Google Calendar.";
      setSyncError(detail);
    }
  };
