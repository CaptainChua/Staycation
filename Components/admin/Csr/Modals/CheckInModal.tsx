...
import {
  ...
  Loader2, Calendar, MapPin, User, CreditCard, Banknote,
} from "lucide-react";
...
export default function CheckInModal({ booking, onClose, onConfirm, isLoading = false }: CheckInModalProps) {
  ...
  return createPortal(
    <>
      ...
      <div
        ref={modalRef}
        className="fixed z-[9991] w-full max-w-lg bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col"
          ...
          {currentStep === 1 && (
            <>
              <div className="flex items-center gap-3 p-3 bg-green-50 dark:bg-green-900/10 rounded-lg border border-green-200 dark:border-green-900/30">
                ...
                <div>
                  <p className="text-sm font-medium text-green-700 dark:text-green-400">Security deposit collected</p>
                  <p className="text-xs text-green-600 dark:text-green-500">{formatCurrency(booking.security_deposit || 1000)} via {paymentMethod}</p>
                </div>
              </div>
