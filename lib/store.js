...
let cache = null;     // in-memory mirror of every shared key
...
function firestorePersistKey(key) {
  return db.collection(FS_COLLECTION).doc(key)
    .set({ json: JSON.stringify(cache[key]) });
}
