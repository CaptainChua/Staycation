...
module.exports = {
  ...
  // one-time diagnostic: which bucket names exist / what errors / the real list
  async storageDebug() {
    const out = { backend, bucketActive: !!bucket, candidates: [], listed: null, listError: null };
    ...
    try {
      ...
      for (const name of cands) {
        try { const [ex] = await getStorage().bucket(name).exists(); out.candidates.push({ name, exists: ex }); }
        catch (e) { out.candidates.push({ name, error: String(e.message || e).split("\n")[0] }); }
      }
      try {
        const [buckets] = await getStorage().bucket(pid + ".appspot.com").storage.getBuckets();
        out.listed = buckets.map(b => b.name);
      } catch (e) { out.listError = String(e.message || e).split("\n")[0]; }
    } catch (e) { out.listError = String(e.message || e); }
    return out;
  },
