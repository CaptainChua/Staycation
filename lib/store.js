...
let imageCache = {};  // id → base64 data-url, for fast reassembly without per-read fetches
...
// returns a copy of value with every "shphimg::<id>" reference swapped for its image
function inlineImages(value, map) {
  return mapStrings(value, s => {
    if (typeof s === "string" && s.startsWith(IMG_PREFIX)) {
      const id = s.slice(IMG_PREFIX.length);
      return (map && map[id] != null) ? map[id] : s; // leave the ref if the image is missing
    }
    return s;
  });
}
...
async function firestoreHydrate() {
  const d = defaults();
  // 1) load every offloaded image into memory so reads can reassemble instantly
  imageCache = {};
  const imgSnap = await db.collection(IMG_COLLECTION).get();
  imgSnap.forEach(doc => {
    const data = doc.data();
    if (data && data.data != null) imageCache[doc.id] = data.data;
  });
  // 2) load the shared docs (which hold image references) and inline the images
  ...
  for (const k of SHARED_KEYS) {
    if (k in found) {
      out[k] = inlineImages(found[k], imageCache); // restore full base64 images
    } else {
...
// offload any new base64 images to their own docs, then store the shared doc
// (which now only holds tiny references). cache[key] keeps the full images.
async function firestorePersistKey(key) {
  const { clean, images } = extractImages(cache[key]);
  const writes = [];
  for (const id in images) {
    if (imageCache[id] !== undefined) continue;          // already stored — skip (dedupe)
    if (Buffer.byteLength(images[id]) > IMG_MAX_BYTES) {  // too big for one doc — log & skip
      console.warn(`[store] image ${id} exceeds 1MB — left out of the cloud`);
      continue;
    }
    imageCache[id] = images[id];
    writes.push(
      db.collection(IMG_COLLECTION).doc(id).set({ data: images[id] })
