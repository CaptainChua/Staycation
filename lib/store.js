...
async function firestorePersistKey(key) {
  ...
  for (const id in images) {
    ...
    if (bucket) {
      // Cloud Storage — no size limit, uses the bucket you purchased
      writes.push(
        uploadImage(id, dataUrl)
          .then(() => knownImageIds.add(id))
          .catch(e => console.error(`[store] Storage upload failed (${id}):`, e.message))
      );
    } else {
      // fallback: legacy Firestore base64 doc (still capped at 1MB)
      if (Buffer.byteLength(dataUrl) > IMG_MAX_BYTES) {
        console.warn(`[store] image ${id} exceeds 1MB and no Storage bucket — skipped`);
        continue;
      }
      knownImageIds.add(id);
      writes.push(
        db.collection(IMG_COLLECTION).doc(id).set({ data: dataUrl })
          .catch(e => console.error(`[store] image write failed (${id}):`, e.message))
      );
    }
  }
  if (writes.length) await Promise.all(writes);
...
module.exports = {
  ...
  // diagnostic: detect the bucket, then upload/read/delete a tiny test image to
  // confirm the write path works end-to-end.
  async storageDebug() {
    const out = { backend, bucketActive: !!bucket, candidates: [], writeTest: null };
    if (backend !== "firestore") return out;
    let liveBucket = bucket;
    try {
      const { getStorage } = require("firebase-admin/storage");
      const account = loadServiceAccount();
      const pid = account ? account.project_id : null;
      const cands = [STORAGE_BUCKET_ENV, pid + ".firebasestorage.app", pid + ".appspot.com"].filter(Boolean);
      for (const name of cands) {
        try {
          const b = getStorage().bucket(name);
          const [ex] = await b.exists();
          out.candidates.push({ name, exists: ex });
          if (ex && !liveBucket) liveBucket = b;
        } catch (e) { out.candidates.push({ name, error: String(e.message || e).split("\n")[0] }); }
      }
    } catch (e) { out.writeTest = "detect error: " + String(e.message || e); return out; }
    if (liveBucket) {
      try {
        const tiny = Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==", "base64");
        const f = liveBucket.file(IMG_DIR + "/__selftest__");
        await f.save(tiny, { resumable: false, contentType: "image/png" });
        const [ex] = await f.exists();
        await f.delete().catch(() => {});
        out.writeTest = ex ? "ok" : "uploaded-but-not-found";
      } catch (e) { out.writeTest = "FAILED: " + String(e.message || e).split("\n")[0]; }
    } else {
      out.writeTest = "no bucket found";
    }
    return out;
  },
