...
|--------|----------|-------------|
| GET | `/api/admin/activity-logs` | Get all activity logs (supports filters) |
| POST | `/api/admin/activity-logs` | Create activity log |
| DELETE | `/api/admin/activity-logs?id=` | Delete activity log |
| GET | `/api/admin/activity-stats` | Get employee activity statistics |
...
```
app/api/admin/activity-logs/route.ts          ← GET all, POST create, DELETE
app/api/admin/activity-stats/route.ts         ← GET stats
backend/controller/activityLogController.ts   ← All controller logic
redux/api/activityLogApi.ts                   ← RTK Query API slice
```
...
### `backend/controller/activityLogController.ts`
...
|------|--------|----------|
| `useGetActivityLogsQuery(params?)` | GET | `/api/admin/activity-logs` |
| `useGetActivityStatsQuery()` | GET | `/api/admin/activity-stats` |
| `useCreateActivityLogMutation()` | POST | `/api/admin/activity-logs` |
| `useDeleteActivityLogMutation()` | DELETE | `/api/admin/activity-logs?id=` |

---

## Query Parameters — `GET /api/admin/activity-logs`

| Param | Type | Description |
|-------|------|-------------|
| `employee_id` | string | Filter by specific employee |
| `action_type` | string | Filter by type (uppercased before query) |
| `search` | string | ILIKE search on activity_type, description, entity_type, first_name, last_name |
| `date_range` | `today` \| `yesterday` \| `last_7_days` \| `last_30_days` \| `custom` \| `all` | Preset date filter (Manila timezone) |
| `start_date` | string | ISO date — used when `date_range=custom` |
| `end_date` | string | ISO date — used when `date_range=custom` |
...
- Logs are stored in `employee_activity_logs` (primary) and `staff_activity_logs` (legacy).
- `createActivityLog` accepts either `employee_id` (new path → `employee_activity_logs`) or `employment_id` (legacy fallback).
- `deleteActivityLog` deletes from `staff_activity_logs`, not the newer table.
- All timestamps are returned in **Asia/Manila** timezone.
- `getActivityStats` queries the `employees` table, not the log table — it returns counts of active CSR, active Cleaners, and inactive staff.
- The `activityLogApi` slice uses `baseUrl: "/api/admin"` (not `/api`), so URLs in the slice are relative paths like `/activity-logs`.
